static const std::unordered_map<Unicode::String, Wide::Lexer::TokenType> reserved_words(
    []() -> std::unordered_map<Unicode::String, Wide::Lexer::TokenType>
    {
        ...
        result[L"activate"] = Wide::Lexer::TokenType::activate;
        result[L"deactivate"] = Wide::Lexer::TokenType::deactivate;
...
public:
    ...
    std::map<std::wstring, TokenType> Analyze(const std::vector<Token*>& tokens) {
...
private:
    ...
    void HandleIdentifier(const Token& token) {
        ...
        // For example, you can check if the identifier is a reserved word
        if (reserved_words.find(token.value) != reserved_words.end()) {
            result[token.value] = reserved_words[token.value];
        } else {
            // Perform additional checks or actions for user-defined identifiers
        }
    }

    void HandleNumber(const Token& token) {
        // Convert token.value to a numeric value and process accordingly
        // For example, you can check if the number is an integer or a floating-point value
    }

    void HandleString(const Token& token) {
        // Process string token (token.value) accordingly
        // For example, you can check if the string is a valid character sequence
    }

    void HandlePunctuation(const Token& token) {
        // Handle punctuation tokens here
        // For example, you can check if the punctuation is used correctly in the context of the language
    }

    std::map<std::wstring, TokenType> result;
    const std::unordered_map<Unicode::String, Wide::Lexer::TokenType> reserved_words;
};
    return tokens;